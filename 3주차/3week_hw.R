...
dataset <- read.csv("weight_height.csv")
...
#tmtest <- tc1
#tmtest[tmtest$Gender=="Male",]$Gender="1"
#tmtest
#mdl <- glm(Gender ~ weight+Height, data = tmtest,family = binomial())
#mdl
#slope <- coef(mdl)[2]/(-coef(mdl)[3])
#slope
#int <- coef(mdl)[1]/(-coef(mdl)[3]) 
#int

library(e1071)
tc1
iris.subset = subset(tc1, 
                     select=c("Height", "weight", "Gender")
                     , Gender %in% c("Male", "Female"))
iris.subset
plot(x=iris.subset$Height, y=iris.subset$weight, col=iris.subset$Gender, pch=19)
svm.model = svm(Gender~., data = tc1, kernel = "linear",
                scale = T)
svm.model
